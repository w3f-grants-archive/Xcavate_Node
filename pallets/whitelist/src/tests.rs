...
fn add_to_whitelist_fails_when_user_already_added() {
	new_test_ext().execute_with(|| {
		System::set_block_number(1);
		assert_ok!(Whitelist::add_to_whitelist(RuntimeOrigin::root(), 1));
		assert_noop!(Whitelist::add_to_whitelist(RuntimeOrigin::root(), 1), Error::<Test>::AccountAlreadyWhitelisted);
	});
}
...
fn remove_from_whitelist_fails() {
	new_test_ext().execute_with(|| {
		System::set_block_number(1);
		assert_noop!(Whitelist::remove_from_whitelist(RuntimeOrigin::root(), 1), Error::<Test>::UserNotInWhitelist);
	});
}
