...
fn upgrade_price_fails_if_not_owner() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(NftMarketplace::listed_nfts().len(), 0);
		assert_ok!(NftMarketplace::list_nft(RuntimeOrigin::signed([1; 32].into()), 0, 27, 100));
		assert_noop!(NftMarketplace::upgrade_listing(RuntimeOrigin::signed([4; 32].into()), 0, 27, 300), Error::<Test>::NoPermission);
	})
}
...
fn upgrade_object_and_distribute_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(NftMarketplace::upgrade_object(RuntimeOrigin::signed([0; 32].into()), 0, 2_000_000));
...
fn upgrade_single_nft_from_listed_object_fails() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(NftMarketplace::upgrade_listing(RuntimeOrigin::signed([0; 32].into()), 0, 27, 300), Error::<Test>::NoPermission);
	})
}
...
fn upgrade_object_for_relisted_nft_fails() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(NftMarketplace::upgrade_object(RuntimeOrigin::signed([0; 32].into()), 0, 300), Error::<Test>::NftAlreadyRelisted);
	})
}
...
fn upgrade_unknown_collection_fails() {
	new_test_ext().execute_with(|| {
		System::set_block_number(1);
		assert_ok!(Whitelist::add_to_whitelist(RuntimeOrigin::root(), [0; 32].into()));
		assert_noop!(NftMarketplace::upgrade_object(RuntimeOrigin::signed([0; 32].into()), 0, 300), Error::<Test>::CollectionNotFound);
	})
}