...
pub mod pallet {
		...
		pub fn stake(
				...
				>= total_amount_loan
			{
				...
				let queue_value =
					...
					let timestamp = <T as pallet::Config>::TimeProvider::now().as_secs();
					let queue_ledger = QueueLedgerAccount {
						staker: staker.clone(),
						locked: Self::u128_to_balance_option(queue_value).unwrap_or_default(),
						timestamp,
					};
					Self::update_queue_ledger(queue_index, queue_ledger);
					QueueCount::<T>::put(queue_index);
					...
					let ledger = LedgerAccount {
						staker: staker.clone(),
						locked: Self::u128_to_balance_option(staking_value).unwrap_or_default(),
						timestamp,
					};
		...
		pub fn unstake(
			...
			if ledger.locked.is_zero() {
				...
				let index = active_stakings
					.iter()
					.position(|x| *x == staking_index.clone())
	...
	impl<T: Config> Pallet<T> {
		...
		fn available_staking_balance(
			staker: &T::AccountId,
			ledger: &LedgerAccount<BalanceOf<T>, T>,
		) -> BalanceOf<T> {
		...
		fn update_ledger(staking_index: StakingIndex, ledger: LedgerAccount<BalanceOf<T>, T>) {
			if ledger.locked.is_zero() {
				...
				if Self::u128_to_balance_option(locking_amount).is_ok() {
					...
					Ledger::<T>::insert(staking_index, ledger);
				} else {
					Ledger::<T>::insert(staking_index, ledger);
				}
			}
